...
import { createMailbox, type Mailbox, type MailboxOptions } from './mailbox';

export { PROTOCOL_VERSION, protocolEnvelopeSchema };
export { createMailbox } from './mailbox';
...
const DEFAULT_MAX_BODY_BYTES = Number(process.env.MAILBOX_MAX_BODY_BYTES ?? String(1024 * 1024));
...
if (require.main === module) {
  ...
  const options: MailboxOptions = {
    ...
    maxBodyBytes: Number.isFinite(DEFAULT_MAX_BODY_BYTES) ? DEFAULT_MAX_BODY_BYTES : 1024 * 1024,
  };
